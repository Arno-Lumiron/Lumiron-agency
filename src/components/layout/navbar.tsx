...
export function Navbar() {
	...
	return (
		...
		>
			<nav
				className="max-w-7xl mx-auto px-5 md:px-10 flex items-center justify-between h-14"
				aria-label="Navigation principale"
			>
				{/* Logo */}
				<Link
					href="/"
					aria-label="Lumiron — Retour à l'accueil"
				...
				{/* Desktop links */}
				<ul
					className="hidden md:flex gap-8 text-[15px] text-encre list-none m-0 p-0"
					role="list"
				>
					{NAV_LINKS.map((link) => (
						<li key={link.href}>
				...
				<div className="flex items-center gap-3">
					<Btn
						variant="primary"
						href={CALENDLY_URL}
						className="hidden md:inline-flex text-[11px] px-3.5 py-2"
					>
						Réserver une démo →
					</Btn>
