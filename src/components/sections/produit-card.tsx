...
export function ProduitCard({ product, direction = 'left' }: ProduitCardProps) {
  return (
    <AnimatedReveal
      ...
      className="bg-ivoire p-[60px_50px_50px] flex flex-col gap-6 relative md:p-10"
    >
      {/* Name + index */}
      <div className="flex items-baseline justify-between gap-4">
        <span className={cn(
          ...
          {product.id}
          <em className="italic text-coral font-normal">.</em>
        </span>
      ...
      <div className="flex flex-col gap-2">
        <span className="text-[11px] tracking-[.18em] uppercase text-gris font-medium">Pour qui ?</span>
        <span className="text-[14.5px] leading-[1.5] text-encre">{product.audience}</span>
      </div>

      {/* Features */}
      <div className="flex flex-col gap-2">
        <span className="text-[11px] tracking-[.18em] uppercase text-gris font-medium">Ce qu'il fait</span>
        <ul className="list-none p-0 m-0">
          {product.features.map((f, i) => (
            ...
            >
              <span className="text-coral font-medium" aria-hidden="true">→</span>
              <span>{f.text}</span>
            </li>
          ))}
        </ul>
      ...
      <div className="mt-auto pt-7 border-t border-encre flex flex-col gap-4">
        <div className="flex flex-col gap-2">
          <span className="text-[11px] tracking-[.18em] uppercase text-gris font-medium">La promesse</span>
          <p className="text-[18px] font-medium tracking-[-0.01em] leading-[1.25] m-0">
            {product.promise}{' '}
            <em className="text-coral italic font-normal">{product.promiseHighlight}</em>
          </p>
        </div>
